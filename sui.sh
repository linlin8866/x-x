...
install_s-ui() {
    ...
    local local_tar="/root/s-ui-linux-${arch_name}.tar.gz"

    # 检查本地压缩包是否存在
    if [[ ! -f "${local_tar}" ]]; then
        echo -e "${red}Error: Local package not found -> ${local_tar}${plain}"
        exit 1
    fi
    echo -e "${green}Found local package: ${local_tar}${plain}"

    # 拷贝到/tmp并解压
    cd /tmp/
    cp -f "${local_tar}" /tmp/
    tar zxvf s-ui-linux-${arch_name}.tar.gz
    rm -f s-ui-linux-${arch_name}.tar.gz
    ...
    echo -e "${green}s-ui local installation finished, running now...${plain}"
